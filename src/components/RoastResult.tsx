  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: `Check out this ${result.intensity} code roast from ${profileLabels[result.profile]}:\n\n${result.response}`,
      ...
      <div className="p-6">
        ...
        <div className="mb-6">
          <div className="flex items-center mb-4">
            <span className={`text-lg font-bold ${intensityColors[result.intensity]}`}>
              {result.intensity.charAt(0).toUpperCase() + result.intensity.slice(1)} Roast
            </span>
          ...
          <div className={`p-6 rounded-lg transition-all duration-300 
            ...
            ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}
            ${result.intensity === 'mild' ? 'border-blue-500' : 
              result.intensity === 'medium' ? 'border-yellow-500' :
              result.intensity === 'spicy' ? 'border-orange-500' : 'border-red-500'}`}>
